...
class TestIndex:
    ...
    def test_similarity_expr_reload(self, small_img_tbl: pxt.Table, reload_tester: ReloadTester) -> None:
        skip_test_if_not_installed('transformers')
        t = small_img_tbl
        sample_img = t.select(t.img).head(1)[0, 'img']

        # no index to do similarity query
        ...
        # add an embedding index on column 'img' - name autogenerated to be 'idx3'
        t.add_embedding_index('img', image_embed=clip_img_embed, string_embed=clip_text_embed)
        # similarity query should now work
        df = t.select(t.img.localpath).order_by(t.img.similarity(sample_img), asc=False).limit(1)
        _ = df.collect()
        # add another named index on column 'img'
        t.add_embedding_index('img', idx_name='img_idx', image_embed=clip_img_embed, string_embed=clip_text_embed)
        # similarity query should now fail because there are multiple indices
        with pytest.raises(pxt.Error) as exc_info:
            _ = t.select(t.img.localpath).order_by(t.img.similarity(sample_img), asc=False).limit(1).collect()
        ...
        df = t.select(t.img.localpath).order_by(t.img.similarity(sample_img, idx='idx3'), asc=False).limit(1)
        _ = df.collect()
        df = t.select(t.img.localpath).order_by(t.img.similarity(sample_img, idx='img_idx'), asc=False).limit(1)
        _ = df.collect()
        # now run the query that specifies index to use via ReloadTester and save its result
        res = reload_tester.run_query(df)
        # verify that the result is the same as the original query
        # PXT-382: this is currently failing with multiple indices error
        # RCA: the index name is a runtime parameter, and is not part of the SimilarityExpr
        # components that are persisted. Fix needs discussion.
